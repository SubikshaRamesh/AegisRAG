...
FRONTEND_DIST_DIR_CANDIDATES = [
    Path("insight-hub/dist"),
    Path("frontend/insight-hub/dist"),
]
FRONTEND_DIST_DIR = next(
    (path for path in FRONTEND_DIST_DIR_CANDIDATES if path.exists()),
    FRONTEND_DIST_DIR_CANDIDATES[0],
)
...
if not IS_PRODUCTION:
    ...
    logger.info("Running in development mode: CORS enabled for http://localhost:5173")
else:
    logger.info("Running in production mode: static frontend serving enabled")
...
async def spa_fallback_middleware(request: Request, call_next):
    """Serve index.html for client-side routes in production SPA deployments."""
    response = await call_next(request)
    ...
    # Avoid rewriting asset requests (e.g. /assets/app.js, /favicon.ico)
...
async def stream_query_endpoint(
    ...
    Tokens are streamed using Server-Sent Events (SSE).
    """
    ...
    try:
        ...
        logger.info(
            f"[{correlation_id}] Stream Query START | "
            f"Chat: {chat_id} | "
        ...
        logger.debug(
            f"[{correlation_id}] Retrieval completed in {retrieval_time:.3f}s"
        )
        ...
        async def event_stream():
            ...
            try:
                ...
                logger.error(
                    f"[{correlation_id}] Streaming error: {e}",
                    exc_info=True,
                )
        ...
        async def generate():
            ...
            yield f"data: {json.dumps(metadata_json)}\n\n"
...
if IS_PRODUCTION:
    if not FRONTEND_DIST_DIR.exists():
        logger.warning(
            "Production mode enabled, but frontend build directory was not found at: %s",
            FRONTEND_DIST_DIR,
        )
    else:
        app.mount(
